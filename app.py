...
h1, h2, h3, h4 { font-family: 'Sora', sans-serif; color: var(--text-primary); }
...
.badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 0.7rem; font-weight: 600; letter-spacing: 0.04em; margin: 2px; }
...
.section-header { display: flex; align-items: center; justify-content: space-between; margin: 1.5rem 0 1rem 0; }
.section-title { font-family: 'Sora', sans-serif; font-size: 1.2rem; font-weight: 700; color: var(--text-primary); }
.view-all-link { font-size: 0.82rem; color: var(--primary-light); text-decoration: none; font-weight: 500; }

.sidebar-brand { display: flex; align-items: center; gap: 10px; padding: 0.5rem 0 1.5rem 0; border-bottom: 1px solid var(--border); margin-bottom: 1rem; }
.brand-name { font-family: 'Sora', sans-serif; font-size: 1.3rem; font-weight: 800; color: white; }
.brand-name span { color: var(--primary-light); }
.brand-tag { font-size: 0.65rem; color: var(--text-secondary); letter-spacing: 0.1em; text-transform: uppercase; }
...
.reason-chip {
  ...
  background: rgba(108,63,224,0.1); border: 1px solid rgba(108,63,224,0.25);
  border-radius: 20px; padding: 4px 12px; font-size: 0.75rem; color: var(--primary-light); margin: 3px;
}
...
.sr-divider { height: 1px; background: linear-gradient(90deg, transparent, var(--border), transparent); margin: 1.2rem 0; }

.rank-badge {
  display: inline-flex; align-items: center; justify-content: center;
  width: 28px; height: 28px; border-radius: 8px;
  background: linear-gradient(135deg, var(--primary), var(--primary-dark));
  color: white; font-family: 'Sora', sans-serif; font-size: 0.8rem; font-weight: 700;
}
...
# ── Auto-setup: generate data + model if missing ──────────────────────────────
@st.cache_resource(show_spinner=False)
def setup_and_load():
    """
    First-run setup:
    1. Generate synthetic data if CSVs are missing
    2. Build recommendation model if .pkl is missing
    3. Return loaded engine + dataframes
    """
...
with st.sidebar:
    ...
    st.markdown("""<div style="font-size:0.8rem;color:#9f87c4;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:0.7rem;">Top Categories</div>""", unsafe_allow_html=True)

    for cat, pct, color in [("Electronics",40,"#6c3fe0"),("Fashion",20,"#f43f5e"),
                              ("Home & Kitchen",15,"#10b981"),("Books",10,"#f59e0b"),("Others",15,"#0ea5e9")]:
        st.markdown(f"""
...
# ── Load (with first-run progress) ───────────────────────────────────────────
data_loaded = False
engine = products_df = users_df = activities_df = None

try:
    need_setup = not os.path.exists(os.path.join(ROOT, "data", "products.csv")) or \
                 not os.path.exists(os.path.join(ROOT, "models", "recommendation_model.pkl"))

    if need_setup:
        with st.status("🚀 First-time setup — this takes ~60 seconds and never repeats...", expanded=True) as status:
...
if page == "Home":
    ...
    import pages.search as p; p.render()
elif page == "Recommendations":
    import pages.recommendations as p; p.render()
elif page == "Wishlist":
    import pages.wishlist as p; p.render()
elif page == "Profile":
    import pages.profile as p; p.render()
elif page == "Analytics":
    import pages.analytics as p; p.render()
elif page == "Admin":
    import pages.admin as p; p.render()
